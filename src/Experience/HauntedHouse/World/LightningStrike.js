...
export default class LightningStrikes
{
	...
	update()
	{
		if (this.experience.home.button.clicked && !this.launched)
		{
			this.launchLightningStrike();
			// this.animationligntningStrike();
		}
		this.animationligntningStrike();
	...
	animationligntningStrike()
	{
		this.lightningStrikeGeometry.update(this.timer.elapsed * 0.1);
		if (this.timer.elapsed > 2 && this.timer.elapsed < 18)
		{
			if (Math.random() > 0.93 || this.flash.power > 100 || this.timer.elapsed < 10) {
